...
class Star {
  ...
  reset() {
    ...
    this.size =
      (Math.random() * 1.5 + 0.5) * (this.layer / config.parallaxLayers);
...
function drawConstellations() {
  constellations.forEach((constellation) => {
    ...
    const color = isHovered
      ? config.constellationHoverColor
      : config.constellationColor;
...
function isPointInConstellation(x, y, constellation) {
  ...
  if (
    x < constellation.bounds.minX ||
    x > constellation.bounds.maxX ||
    y < constellation.bounds.minY ||
    y > constellation.bounds.maxY
  ) {
    return false;
  }
